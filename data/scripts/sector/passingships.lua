...
local BlackMarketUT = include("internal/dlc/blackmarket/lib/blackmarketutility")
...
-- family ship
function PassingShips.createPassingFamilyShip()

    local faction = BlackMarketUT.getFamilyFaction()
    -- this is the position where the trader spawns
    local dir = random():getDirection()
    local pos = dir * 1500
    ...
    local names =
    {
        "Family Man"%_T,
        "Godchild"%_T,
        -- we have to translate on the spot here since _T doesn't remove the translation context comment
        -- not too bad though because it's a name anyway and won't be translated usually
        "Vito /* Reference to Vito Corleone from The Godfather */"%_t,
    }
    ...
    local onFinished = function(ship)
        -- finalize ship
        ship:addScriptOnce("ai/passsector.lua", destination)
        ship:addScriptOnce("entity/utility/deleteonhitbyshots.lua")

        ship:setValue("passing_ship", true)
        ship.name = randomEntry(random(), names)

        local lines = PassingShips.getChatterLinesByShipType(PassingShipType.FamilyShip)
        ship:addScriptOnce("data/scripts/entity/utility/radiochatter.lua", lines, 65, 85, random():getInt(5, 45), true)
    end
...
-- commune ship
function PassingShips.createPassingCommuneShip()

    local faction = BlackMarketUT.getCommuneFaction()
...
-- cavaliers ship
function PassingShips.createPassingCavaliersShip(faction)

    local faction = BlackMarketUT.getCavaliersFaction()
...
function PassingShips.getChatterLinesByShipType(type)
    ...
    elseif type == PassingShipType.FamilyShip then
        result =
        {
            "We have some offers you can't refuse!"%_T,
            "Don't try to rip us off, we are protected by the Family."%_T,
            "The Family takes care of every trader that is willing to pay for protection."%_T,
            "We are not only trading goods but also favors."%_T,
        }
    elseif type == PassingShipType.CommuneShip then
        result =
        {
            "Join us into a just and beautiful future!"%_T,
            "We open our arms for every newcomer."%_T,
            "The Commune will bring peace and happiness to the galaxy."%_T,
        }
    elseif type == PassingShipType.CavaliersShip then
        result =
        {
            "Come and enroll to our glorious order!"%_T,
            "Help us unite the galaxy under the generous rule of the Emperor!"%_T,
            "Pirates beware, the Cavaliers are here!"%_T,
            "We protect the weak and the poor from any danger!"%_T,
        }
    else
