...
function AsteroidPlanGenerator:makeMonolithAsteroidPlan(size, material, flags)
    ...
    local starters = {}
    table.insert(starters, plan:addBlock(vec3(1, 0, 0), variedSize(), 0, -1, color, material, matrix, center))
    table.insert(starters, plan:addBlock(vec3(2, 0, 0), variedSize(), starters[1], -1, color, material, matrix, center))
    table.insert(starters, plan:addBlock(vec3(3, 0, 0), variedSize(), starters[2], -1, color, material, matrix, center))

    for x, starter in pairs(starters) do
        local previous = starter

        for i = 1, 9 do
            previous = plan:addBlock(vec3(x, 1 * i, 0), variedSize(), previous, -1, color, material, matrix, center)
        end
    end
...
function AsteroidPlanGenerator:makeCuboidAsteroidPlan(size, material, flags)
    ...
    local starters = {}
    table.insert(starters, {x=1, z=1, b=plan:addBlock(vec3(1, 0, 1), variedSize(), 0, -1, color, material, matrix, center)})
    table.insert(starters, {x=2, z=1, b=plan:addBlock(vec3(2, 0, 1), variedSize(), starters[1], -1, color, material, matrix, center)})
    table.insert(starters, {x=3, z=1, b=plan:addBlock(vec3(3, 0, 1), variedSize(), starters[2], -1, color, material, matrix, center)})
    table.insert(starters, {x=1, z=2, b=plan:addBlock(vec3(1, 0, 2), variedSize(), starters[1], -1, color, material, matrix, center)})
    table.insert(starters, {x=2, z=2, b=plan:addBlock(vec3(2, 0, 2), variedSize(), starters[4], -1, color, material, matrix, center)})
    table.insert(starters, {x=3, z=2, b=plan:addBlock(vec3(3, 0, 2), variedSize(), starters[5], -1, color, material, matrix, center)})
    table.insert(starters, {x=1, z=3, b=plan:addBlock(vec3(1, 0, 3), variedSize(), starters[4], -1, color, material, matrix, center)})
    table.insert(starters, {x=2, z=3, b=plan:addBlock(vec3(2, 0, 3), variedSize(), starters[6], -1, color, material, matrix, center)})
    table.insert(starters, {x=3, z=3, b=plan:addBlock(vec3(3, 0, 3), variedSize(), starters[7], -1, color, material, matrix, center)})

    for _, starter in pairs(starters) do
        local previous = starter.b

        for i = 1, 2 do
            previous = plan:addBlock(vec3(starter.x, i, starter.z), variedSize(), previous, -1, color, material, matrix, center)
        end
    end
