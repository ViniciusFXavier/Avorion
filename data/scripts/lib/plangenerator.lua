...
function PlanGenerator.makeCrewShuttlePlan(factionIndex, seed, material)
    ...
    for i = 0, plan.numBlocks - 1 do
        ...
        if block.box.position.z >= 0 then
            container:addBlock(block.box.position + vec3(0, 0, maxZ), block.box.size, container.rootIndex, -1, block.color, block.material, block.orientation, block.blockIndex)
        else
            container:addBlock(block.box.position + vec3(0, 0, minZ), block.box.size, container.rootIndex, -1, block.color, block.material, block.orientation, block.blockIndex)
        end
    end
...
function PlanGenerator.makeGenericContainerPlan(colors_in, blockType)
    ...
    if math.random() < 0.5 then
        ...
        local sideBlock1 = plan:addBlock(vec3(0, 1 + size / 2, 0), vec3(size, size, size), root, -1, color, Material(), Matrix(), blockType)
        local sideBlock2 = plan:addBlock(vec3(0, -(1 + size / 2), 0), vec3(size, size, size), root, -1, color, Material(), Matrix(), blockType)
        ...
        if math.random() < 0.5 then
            ...
            local anchorBlock1 = plan:addBlock(newPos, vec3(size - 0.4, 0.2, size), sideBlock1, -1, color, Material(), Matrix(), blockType)

            local newPos = plan:getBlock(sideBlock2).box.position
            newPos.y = newPos.y - (size / 2 + 0.1)
            local anchorBlock2 = plan:addBlock(newPos, vec3(size - 0.4, 0.2, size), sideBlock2, -1, color, Material(), Matrix(), blockType)

            -- add glow strips
            local newPos = plan:getBlock(anchorBlock1).box.position
            newPos.x = newPos.x + (size / 2 - 0.1)
            plan:addBlock(newPos, vec3(0.2, 0.2, size), anchorBlock1, -1, glowColor, Material(), Matrix(), BlockType.Glow)
            ...
            newPos.x = newPos.x + (size / 2 - 0.1)
            plan:addBlock(newPos, vec3(0.2, 0.2, size), anchorBlock2, -1, glowColor, Material(), Matrix(), BlockType.Glow)
    ...
    while added < maxAdded do
        ...
        if math.random() < 0.4 and added > maxAdded and glowAdded == false and pairsCounter < 6 then
            if #blockPairs > 0 then
                ...
                local centerA = plan:addBlock(vec3(0, 0, added + 0.1), vec3(size - 0.4, size - 0.4, 0.2), blockPairs[#blockPairs].a, -1, color, Material(), Matrix(), blockType)
                local centerB = plan:addBlock(vec3(0, 0, -(added + 0.1)), vec3(size - 0.4, size - 0.4, 0.2), blockPairs[#blockPairs].b, -1, color, Material(), Matrix(), blockType)

                -- add the side pieces
                local newPos = plan:getBlock(centerA).box.position
                newPos.y = newPos.y + (size/2 - 0.1)
                local topA = plan:addBlock(newPos, vec3(size - 0.4, 0.2, 0.2), centerA, -1, color, Material(), Matrix(), blockType)

                local newPos = plan:getBlock(centerA).box.position
                newPos.y = newPos.y - (size/2 - 0.1)
                local bottomA = plan:addBlock(newPos, vec3(size - 0.4, 0.2, 0.2), centerA, -1, color, Material(), Matrix(), blockType)

                local newPos = plan:getBlock(centerA).box.position
                newPos.x = newPos.x - (size/2 - 0.1)
                plan:addBlock(newPos, vec3(0.2, size - 0.4, 0.2), centerA, -1, color, Material(), Matrix(), blockType)
                ...
                newPos.y = newPos.y + (size/2 - 0.1)
                local topB = plan:addBlock(newPos, vec3(size - 0.4, 0.2, 0.2), centerB, -1, color, Material(), Matrix(), blockType)

                local newPos = plan:getBlock(centerB).box.position
                newPos.y = newPos.y - (size/2 - 0.1)
                local bottomB = plan:addBlock(newPos, vec3(size - 0.4, 0.2, 0.2), centerB, -1, color, Material(), Matrix(), blockType)

                local newPos = plan:getBlock(centerB).box.position
                newPos.x = newPos.x - (size/2 - 0.1)
                plan:addBlock(newPos, vec3(0.2, size - 0.4, 0.2), centerB, -1, color, Material(), Matrix(), blockType)
                ...
                newPos.x = newPos.x + (size/2 - 0.1)
                plan:addBlock(newPos, vec3(0.2, 0.2, 0.2), topA, -1, glowColor, Material(), Matrix(), BlockType.Glow)
                ...
                newPos.x = newPos.x + (size/2 - 0.1)
                plan:addBlock(newPos, vec3(0.2, 0.2, 0.2), bottomA, -1, glowColor, Material(), Matrix(), BlockType.Glow)
                ...
                newPos.x = newPos.x + (size/2 - 0.1)
                plan:addBlock(newPos, vec3(0.2, 0.2, 0.2), topB, -1, glowColor, Material(), Matrix(), BlockType.Glow)
                ...
                newPos.x = newPos.x + (size/2 - 0.1)
                plan:addBlock(newPos, vec3(0.2, 0.2, 0.2), bottomB, -1, glowColor, Material(), Matrix(), BlockType.Glow)
    ...
    for i, blocks in pairs(blockPairs) do
        if math.random() < 0.3 and glowAdded == false then
            ...
            local sideBlock1 = plan:addBlock(newPos, newSize, blocks.a, -1, ColorRGB(1, 1, 1), Material(), Matrix(), blockType)

            -- -x
            local newPos = plan:getBlock(blocks.a).box.position
            newPos.x = newPos.x - (size.x / 2 + newSize.x / 2)
            local sideBlock2 = plan:addBlock(newPos, newSize, blocks.a, -1, ColorRGB(1, 1, 1), Material(), Matrix(), blockType)
            ...
            newPos.x = newPos.x + (size.x / 2 + newSize.x / 2)
            local sideBlock3 = plan:addBlock(newPos, newSize, blocks.b, -1, ColorRGB(1, 1, 1), Material(), Matrix(), blockType)

            -- -x
            local newPos = plan:getBlock(blocks.b).box.position
            newPos.x = newPos.x - (size.x / 2 + newSize.x / 2)
            local sideBlock4 = plan:addBlock(newPos, newSize, blocks.b, -1, ColorRGB(1, 1, 1), Material(), Matrix(), blockType)
            ...
            if math.random() < 0.5 then
                ...
                plan:addBlock(newPos, vec3(0.2, 0.2, 0.5), sideBlock1, -1, glowColor, Material(), Matrix(), BlockType.Glow)

                local newPos = plan:getBlock(sideBlock2).box.position
                newPos.x = newPos.x - 0.1
                plan:addBlock(newPos, vec3(0.2, 0.2, 0.5), sideBlock2, -1, glowColor, Material(), Matrix(), BlockType.Glow)

                local newPos = plan:getBlock(sideBlock3).box.position
                newPos.x = newPos.x + 0.1
                plan:addBlock(newPos, vec3(0.2, 0.2, 0.5), sideBlock3, -1, glowColor, Material(), Matrix(), BlockType.Glow)

                local newPos = plan:getBlock(sideBlock4).box.position
                newPos.x = newPos.x - 0.1
                plan:addBlock(newPos, vec3(0.2, 0.2, 0.5), sideBlock4, -1, glowColor, Material(), Matrix(), BlockType.Glow)
            end
        end
