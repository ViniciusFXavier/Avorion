...
local function new(x, y)
    ...
    instance.chanceForGoodsInWreckage = 0.2
    instance.chanceForUnstrippedWreckage = 0.1 * GameSettings().resourceWreckageFactor
    instance.chanceForCaptainLogWreckage = 0.2

    instance.smallFieldAsteroids = 50
    instance.normalFieldAsteroids = 350
    instance.denseFieldAsteroids = 600

    return setmetatable(instance, SectorGenerator)
end
...
function SectorGenerator:createContainer(plan, worldMatrix, factionIndex)
    plan = plan or PlanGenerator.makeContainerPlan()
...
function SectorGenerator:createContainerField(sizeX, sizeY, circular, position, factionIndex, hackables)
    ...
    for y = 1, sizeY do
        ...
        for x = 1, sizeX do
            ...
            if circular == 1 then
                ...
                if distance2(vec2(x, y), vec2(sizeX / 2, sizeY / 2)) > radius * radius then
                    create = 0
                end
            end
    ...
    for i = 1, hackables do
        ...
        if container:hasScript("internal/dlc/blackmarket/entity/hackablecontainer.lua") then
            i = i - 1
            goto continue
        end
...
-- create hidden treasure asteroid
...
function SectorGenerator:createEmptyBigAsteroid(position)
...
function SectorGenerator:createBigAsteroidEx(position, size, resources)
    ...
    if not resources and (material == Material(MaterialType.Iron) or material == Material(MaterialType.Naonite)) then
        material = Material(MaterialType.Titanium)
    end

    local asteroidPlanGenerator = AsteroidPlanGenerator()
    if Galaxy():sectorInRift(self.coordX, self.coordY) then
        asteroidPlanGenerator:setToRiftStone()
    end

    local plan = asteroidPlanGenerator:makeBigAsteroidPlan(size, resources, material)
...
function SectorGenerator:createDenseAsteroidField(probability)
    local size = getFloat(0.75, 1.25)
    local generator = AsteroidFieldGenerator(self.coordX, self.coordY)
    return generator:createAsteroidFieldEx(self.denseFieldAsteroids * size, 1800 * size, 5.0, 25.0, true, probability);
end

function SectorGenerator:createAsteroidField(probability)
    local size = getFloat(0.75, 1.25)
    local generator = AsteroidFieldGenerator(self.coordX, self.coordY)
    return generator:createAsteroidFieldEx(self.normalFieldAsteroids * size, 1800 * size, 5.0, 25.0, true, probability);
end

function SectorGenerator:createSmallAsteroidField(probability)
    local d = getFloat(0.2, 0.4)
    local size = getFloat(0.5, 1.5)

    local generator = AsteroidFieldGenerator(self.coordX, self.coordY)
    return generator:createAsteroidFieldEx(self.smallFieldAsteroids * size, 1800 * d, 5.0, 25.0, true, probability);
end

function SectorGenerator:createEmptyAsteroidField()
    local size = getFloat(0.75, 1.25)
    local generator = AsteroidFieldGenerator(self.coordX, self.coordY)
    return generator:createAsteroidFieldEx(self.normalFieldAsteroids * size, 1800 * size, 5.0, 25.0, false);
end
...
function SectorGenerator:createEmptySmallAsteroidField()
    ...
    local generator = AsteroidFieldGenerator(self.coordX, self.coordY)
    return generator:createAsteroidFieldEx(self.smallFieldAsteroids * size, 1800 * d, 5.0, 25.0, false);
end
...
function SectorGenerator:createUnstrippedWreckage(faction, plan, breaks, position)
    ...
    for _, plan in pairs(plans) do
        ...
        plan.accumulatingHealth = false
        local wreckage = Sector():createWreckage(plan, position or self:getPositionInSector())

        table.insert(wreckages, wreckage)

        if random():test(self.chanceForGoodsInWreckage) then
            -- add cargo
            local index = math.random(1, #spawnableGoods)
            local g = spawnableGoods[index]
            local good = g:good()

            local maxValue = math.random(500, 3000) * Balancing_GetSectorRichnessFactor(self.coordX, self.coordY);
            local maxVolume = 100
...
function SectorGenerator:createTravelHub(faction)
    local station = self:createStation(faction, "data/scripts/entity/merchants/travelhub.lua");

    station:addScript("data/scripts/entity/merchants/consumer.lua", "Travel Hub"%_t, unpack(ConsumerGoods.TravelHub()))

    return station
end


function SectorGenerator:createRiftResearchCenter(faction)
    local station = self:createStation(faction, "internal/dlc/rift/entity/riftresearchcenter.lua");

    station:addScript("data/scripts/entity/merchants/consumer.lua", "Rift Research Center"%_t, unpack(ConsumerGoods.RiftResearchStation()))

    -- this station has a specialized bulletin board, so remove the general ones
    station:removeScript("bulletinboard.lua")
    station:removeScript("missionbulletins.lua")
    station:removeScript("story/bulletins.lua")

    -- add special rift research shop where players can buy equipment for rift research data
    station:addScriptOnce("internal/dlc/rift/entity/riftresearchshop.lua")
    station:addScriptOnce("internal/dlc/rift/entity/riftresearchutilityshop.lua")
...
function SectorGenerator:makeBeaconDescriptor(position, factionIndex)
    local desc = EntityDescriptor()
    desc:addComponents(
       ComponentType.Plan,
       ComponentType.BspTree,
...
function SectorGenerator:createBeacon(position, faction, text, args)
    ...
    if faction then factionIndex = faction.index end

    local desc = self:makeBeaconDescriptor(position, factionIndex)
...
function SectorGenerator:estimateAsteroidNumbers(normal, small, dense)
    ...
    return math.ceil(normal * self.normalFieldAsteroids + small * self.smallFieldAsteroids + dense * self.denseFieldAsteroids)
end
