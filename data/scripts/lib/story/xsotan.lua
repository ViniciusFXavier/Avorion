...
function Xsotan.getClassification(strength)
    ...
    classifications[3] = {volume = 1.50, damage = 1.5, weapon = WeaponType.Bolter, name = "Corvette /* ship title */"}
    classifications[4] = {volume = 1.75, damage = 2.0, weapon = WeaponType.PulseCannon, name = "Frigate /* ship title */"}
...
function Xsotan.createShip(position, volumeFactor)
    ...
    if random():test(0.1) then
        turret = generator:generateArmed(x, y, 0, Rarity(RarityType.Common))
    else
        turret = generator:generate(x, y, 0, Rarity(RarityType.Common), classification.weapon, nil)
    end
...
function Xsotan.createSummoner(position, volumeFactor)
    ...
    local name, type = ShipUtility.getMilitaryNameByVolume(ship.volume)
    ship:setTitle("${toughness}Xsotan Summoner"%_T, {toughness = "", ship = name})
    ship.crew = ship.idealCrew
    ship.shieldDurability = ship.shieldMaxDurability
...
function Xsotan.createShielded(position, volumeFactor)
    ...
    local faction = Xsotan.getFaction()
    local plan = PlanGenerator.makeXsotanShipPlan(volume, material)
    plan:addBlock(vec3(), vec3(1, 1, 1), plan.rootIndex, -1, ColorRGB(1, 1, 1), material, Matrix(), BlockType.ShieldGenerator)
