...
function getConsumerText(goodsTable)
    ...
    for k, goodName in pairs(goodsTable) do
        if not uniqueGoodsTable[goodName] then
            uniqueGoodsTable[goodName] = true
    ...
    local keyCounter = 0
    local tempTable = {}
    for goodname, _ in pairs (uniqueGoodsTable) do
        local key = math.floor(keyCounter / 3) + 1
        local value = (keyCounter % 3) + 1
        if not tempTable[key] then
            tempTable[key] = {}
        end

        tempTable[key][value] = goodname

        keyCounter = keyCounter + 1
    end

    local counter = 0
    consumerText = consumerText .. string.join(tempTable, ",\n", function(i, str)
        counter = counter + 1
        return string.join(tempTable[counter], ", ")
    end)
