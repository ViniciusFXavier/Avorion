...
function MaintenanceCommand:calculatePrediction(ownerIndex, shipName, area, config)
    ...
    local factor = 0
    for _, perk in pairs({captain:getPerks()}) do
        factor = factor + CaptainUtility.getMaintenanceTimePerkImpact(captain, perk)
    end

    duration = duration + duration * factor
    crewDuration = crewDuration + crewDuration * factor
    baseDuration = baseDuration + baseDuration * factor
