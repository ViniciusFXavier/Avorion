...
function activate(item)
    ...
    -- find biggest radius to make sure passageSize is big enough
    ...
    for _, index in pairs(factions) do
        ...
        if faction.isPlayer or faction.isAlliance then
            for _, ship in pairs({sector:getEntitiesByFaction(index)}) do
                if ship.isShip and ship.radius > minRadius then
                    minRadius = ship.radius
                end
            end
        end
    end
    ...
    wormhole.passageSize = minRadius * 2 + 100 -- a bit extra to be on the safe side
