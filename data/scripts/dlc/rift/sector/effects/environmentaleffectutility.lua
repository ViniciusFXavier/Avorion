...
EnvironmentalEffectUT.data[EffectType.AcidFog] =
{
    ...
    description = "This acid fog corrodes blocks below ${material}. These will suffer constant damage. Shields are useless against the acid fog."%_T,
    ...
    isShipPrepared = function(ship, _, x, y)
        ...
        local recommended = Material(Balancing_GetHighestAvailableMaterial(x, y))
        local materialValue = recommended.value

        local volumeDamageableBlocks = 0
        local volumeSafeBlocks = 0
        for i = 0, numBlocks - 1 do
            local block = plan:getNthBlock(i)
            if block.material.value > materialValue then
...
function EnvironmentalEffectUT.getFormatArguments(effectType, intensity, x, y)
    ...
    if effectType == EffectType.AcidFog then
        ...
        local distanceToCenter = intensity - data.intensity * 1000
        data.material = Material(Balancing_GetHighestAvailableMaterial(distanceToCenter, 0)).name
    end
