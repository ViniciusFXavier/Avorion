...
function RiftObjects.createBatteryStash(position)
    ...
    plan:addBlock(dockPosition, vec3(2, height, 2), plan.rootIndex, -1, ColorRGB(1, 1, 1), Material(MaterialType.Titanium), matrix, BlockType.Dock)
...
function RiftObjects.createBigAsteroid(position)
    ...
    return sectorGenerator:createBigAsteroidEx(position, getFloat(60, 80), true)
end
...
function RiftObjects.createWeaponChamberSwitch(position)
    ...
    local matrix = MatrixLookUp(vec3(0, 1, 0), vec3(1, 0, 0))
    plan:addBlock(dockPosition, vec3(2, 2, 2), plan.rootIndex, -1, ColorRGB(1, 1, 1), Material(MaterialType.Titanium), matrix, BlockType.Dock)
...
function RiftObjects.createInactiveGateActivator(position)
    ...
    local dockPosition = rootBox.position + vec3(0, rootBox.size.y * 0.5, 0) + vec3(0, 1, 0)
    local matrix = MatrixLookUp(vec3(0, 1, 0), vec3(1, 0, 0))
    plan:addBlock(dockPosition, vec3(2, 20, 2), plan.rootIndex, -1, ColorRGB(1, 1, 1), Material(MaterialType.Titanium), matrix, BlockType.Dock)
