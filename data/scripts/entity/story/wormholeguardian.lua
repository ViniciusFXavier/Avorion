...
function WormholeGuardian.onDestroyed()
    local server = Server()
    server:setValue("guardian_respawn_time", 120 * 60)

    -- trigger first spawning of xsotan swarm, but don't override remaining spawn time if it is already set
    server:setValue("xsotan_swarm_time", (server:getValue("xsotan_swarm_time") or 30 * 60))
