...
function CrewBoard.interactionPossible(playerIndex, option)
    ...
    local checkMinimumPopulation = false -- for crew board, skip the minimum population check
    return CheckFactionInteraction(playerIndex, -25000, nil, checkMinimumPopulation)
end
...
function CrewBoard.initialize()
    ...
    if random():test(0.35) then
        if not (MinimumPopulation and not MinimumPopulation.isFulfilled()) then
            local tiers = {}
            tiers[0] = 0.5
            tiers[1] = 0.3
            tiers[2] = 0.2
            ...
            if tier > 0 then
                ...
                elseif station:hasScript("resistanceoutpost.lua") then
                    primary = CaptainUtility.ClassType.Hunter
                elseif station:hasScript("riftresearchcenter.lua") then
                    primary = CaptainUtility.ClassType.Scientist
                end
            end
...
function CrewBoard.refreshUI(lineToReset)
    ...
    if not ship or ship.maxCrewSize == nil or ship.crewSize == nil then
        return
    end
